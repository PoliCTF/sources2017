...
#define BALL_X_MIN_SPEED 8
#define BALL_X_MAX_SPEED 24
#define BALL_Y_MAX_SPEED 18
...
uv_timer_t win_timer;
/*END*/

void closeSender();
void restart(char *ip,int lport);
...
void restart(char *ip,int lport){
    ...
    r=uv_timer_start(&win_timer, send_win, 30000, 100);
    if (r!=0)
        perror(uv_strerror(r));
    started=1;
    srand(time(NULL));
    int signum=(rand() % 2)?-1:1;
    ai_offset =signum * (int)((double)PLAYER_HEIGHT/2 + (double)BALL_DIMENSION/2 - 1);
        ...
        if ((ballx < (SCREEN_WIDTH/2) && (bally + BALL_DIMENSION > p1y && bally < (p1y + PLAYER_HEIGHT))) || ((ballx > (SCREEN_WIDTH/2) && (bally + BALL_DIMENSION > p2y && bally < (p2y + PLAYER_HEIGHT)))))
        {
            ...
            else
            {
                if (ballx + BALL_DIMENSION > (SCREEN_WIDTH - GAP - PLAYER_WIDTH)){
                    ...
                    int signum=(rand() % 2)?-1:1;
                    //////
                    new_ai_offset =signum * (int)((double)PLAYER_HEIGHT/2 + (double)BALL_DIMENSION/2 - 1);
                    //////
...
void on_read(uv_udp_t *req, ssize_t nread, const uv_buf_t *buf, const struct sockaddr *addr, unsigned flags) {
	...
	//fprintf(stderr,"%d Recv -> : '%s' from -> '%s:%d'\n",i++,buf->base,ip,c_port);
        int azione1=-1;
        int azione2=-1;
        int n=0;
        if ((n=sscanf(buf->base,"%d|%d\n",&azione1,&azione2))<2){
            free(buf->base);
            return;
        }
